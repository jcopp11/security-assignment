namespace :ptourist do
  ...
  def create_thing thing, organizer, members, images, inquiry
    ...
    puts "ID de THING: #{thing.id}"
    if inquiry
      inquiry.each do |inq|
        inq['thing_id'] = thing.id
        inq=Inquiry.create!(inq)
        organizer.add_role(Role::ORGANIZER, inq).save
        m=members.map { |member|
          unless (member.id==organizer.id || member.id==mike_user.id)
            member.add_role(Role::MEMBER, inq).save
            member
          end
        }.select {|r| r}
      end
    ...
    puts "#{Thing.count} things created and image created #{ThingImage.count("distinct thing_id")} and #{Inquiry.count} with inquiries"
